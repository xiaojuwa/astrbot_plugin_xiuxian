# AstrBot 文字修仙插件

<p align="center">
  <img src="https://img.shields.io/badge/版本-v2.8.0-blue" alt="Version">
  <img src="https://img.shields.io/badge/框架-AstrBot-green" alt="Framework">
  <img src="https://img.shields.io/badge/数据库版本-v23-orange" alt="DB Version">
  <img src="https://img.shields.io/badge/许可证-AGPL--3.0-red" alt="License">
</p>

一款为 AstrBot 设计的放置类文字修仙游戏插件。从凡人踏入仙途，历经修炼、战斗、探索，终成大道！

> 感谢 [@Zhalslar](https://github.com/Zhalslar) 提供的模块化思路重构了整个插件的结构

---

## 特色功能

| 特色 | 描述 |
|------|------|
| **完整成长体验** | 28级境界体系，从"炼气一层"到"祭道之上" |
| **动态战斗系统** | 基于标签系统动态生成怪物和Boss，每次战斗独一无二 |
| **丰富物品系统** | 60+ 种物品，涵盖丹药、材料、法器、功法 |
| **炼丹/炼器系统** | 收集材料炼制丹药和法器，10级炼丹师/炼器师等级 |
| **宗门社交** | 创建或加入宗门，与道友共谋发展 |
| **多元化玩法** | 排行榜、每日任务、奇遇探索、天劫挑战、悬赏任务 |
| **高度可配置** | 所有数值、物品、怪物、配方均可通过JSON文件自定义 |
| **专业数据管理** | aiosqlite独立存储 + 版本化数据库迁移系统 |

---

## 快速开始

### 安装

1. 在 AstrBot 插件市场搜索 `astrbot_plugin_xiuxian` 安装
2. 或手动克隆到 `AstrBot/data/plugins/` 目录

### 开启修仙之路

```
发送: 我要修仙    → 创建角色
发送: 修仙帮助    → 查看所有指令
发送: 签到        → 领取每日奖励
```

---

## 游戏指令大全

### 入门指引

| 指令 | 说明 |
|:-----|:-----|
| `我要修仙` | 开启修仙之旅，创建角色 |
| `我的信息` | 查看人物详细信息（境界、属性、装备） |
| `签到` | 每日签到，0点刷新 |
| `修仙帮助` | 显示完整帮助信息 |

### 修炼成长

| 指令 | 说明 |
|:-----|:-----|
| `闭关` | 开始闭关修炼（离线挂机） |
| `出关` | 结束闭关，结算修为收益 |
| `突破` | 尝试突破当前境界（需足够修为） |
| `重入仙途` | 花费灵石重置灵根 |

### 坊市物品

| 指令 | 说明 |
|:-----|:-----|
| `商店` | 查看今日坊市商品（每日随机刷新） |
| `我的背包` | 查看背包物品 |
| `购买 <物品名> [数量]` | 购买商品，默认1个 |
| `使用 <物品名> [数量]` | 使用消耗品/装备法器 |
| `出售 <物品名> [数量]` | 出售物品给商店（每日5次，40%价格） |
| `我的装备` | 查看当前装备 |
| `卸下 <部位>` | 卸下装备（武器/防具/饰品） |

### 炼丹/炼器系统 (v2.4.0新增)

| 指令 | 说明 |
|:-----|:-----|
| `炼丹` | 查看炼丹界面（丹炉等级、可用配方） |
| `炼丹 <配方名>` | 炼制丹药 |
| `炼器` | 查看炼器界面（炼器台等级、可用配方） |
| `炼器 <配方名>` | 炼制法器 |
| `升级丹炉` | 升级丹炉（提升成功率和品质） |
| `升级炼器台` | 升级炼器台 |
| `配方 <配方名>` | 查看配方详情（材料、成功率） |
| `配方图鉴` | 查看所有配方列表 |
| `材料图鉴` | 查看所有材料及获取途径 |

### 宗门社交

| 指令 | 说明 |
|:-----|:-----|
| `创建宗门 <宗门名>` | 创建宗门（需要灵石） |
| `加入宗门 <宗门名>` | 加入已有宗门 |
| `我的宗门` | 查看宗门信息 |
| `退出宗门` | 退出当前宗门 |
| `宗门捐献 <数额>` | 向宗门捐献灵石，获得贡献度 |

### 战斗探险

| 指令 | 说明 |
|:-----|:-----|
| `切磋 @某人` | 与其他玩家切磋（需双方满血） |
| `奇斗 @某人 <灵石>` | 带灵石赌注的PVP（胜者赢得灵石） |
| `查看世界boss` | 查看当前活跃的世界Boss |
| `讨伐boss <Boss ID>` | 挑战世界Boss（有冷却时间） |
| `boss战报` | 查看近期Boss击杀战报 |
| `探索秘境 [类型] [难度]` | 进入秘境探索（可选择类型和难度） |
| `前进` | 在秘境中前进一层 |
| `选择 <数字>` | 在秘境事件中做出选择 |
| `离开秘境` | 离开当前秘境 |

### 排行榜

| 指令 | 说明 |
|:-----|:-----|
| `境界排行` | 查看境界排行榜 |
| `财富排行` | 查看灵石排行榜 |
| `战力排行` | 查看综合战力排行榜 |
| `PVP排行` | 查看PVP胜场/胜率排行榜 |
| `我的排名` | 查看个人在各榜单的排名 |

### 每日任务

| 指令 | 说明 |
|:-----|:-----|
| `每日任务` | 查看每日任务进度 |
| `领取任务奖励` | 领取完成的任务奖励 |

> 每日5个任务：闭关、签到、秘境探索、Boss讨伐、切磋，全部完成可领取500灵石全勤奖励

### 奇遇探索

| 指令 | 说明 |
|:-----|:-----|
| `奇遇` | 触发一次奇遇（每日3次） |
| `奇遇状态` | 查看今日奇遇次数 |

### 天劫系统

| 指令 | 说明 |
|:-----|:-----|
| `天劫信息` | 查看天劫信息 |
| `渡劫` | 挑战天劫 |

### 悬赏任务

| 指令 | 说明 |
|:-----|:-----|
| `悬赏榜` | 查看悬赏任务列表 |
| `接取悬赏 <任务名>` | 接取并执行悬赏任务 |
| `悬赏状态` | 查看悬赏完成状态 |

### 交易系统

| 指令 | 说明 |
|:-----|:-----|
| `转账 @某人 <数额>` | 转账灵石给其他玩家（5%税率） |
| `赠送 @某人 <物品> [数量]` | 赠送物品给其他玩家 |
| `我的功法` | 查看已学习的功法及属性加成 |
| `我的buff` | 查看当前激活的战斗buff |

### GM管理员指令 (v2.5.0新增)

> 以下指令仅限管理员使用，用于游戏数据管理和调试
> 默认管理员QQ：1733533842（可在配置中添加更多管理员）
> **v2.5.2更新：所有GM指令现在支持@玩家和直接输入QQ号两种方式**

| 指令 | 说明 |
|:-----|:-----|
| `GM加灵石 @玩家/QQ号 <数额>` | 为玩家增加/扣除灵石（负数为扣除） |
| `GM加修为 @玩家/QQ号 <数额>` | 为玩家增加/扣除修为 |
| `GM设境界 @玩家/QQ号 <境界索引>` | 设置玩家境界（使用`GM境界列表`查看索引） |
| `GM加物品 @玩家/QQ号 <物品名> [数量]` | 为玩家添加物品 |
| `GM设生命 @玩家/QQ号 <数值>` | 设置玩家生命值（同时设置当前和最大生命） |
| `GM设最大生命 @玩家/QQ号 <数值>` | 仅设置最大生命值 |
| `GM设攻击 @玩家/QQ号 <数值>` | 设置玩家基础攻击力 |
| `GM设防御 @玩家/QQ号 <数值>` | 设置玩家基础防御力 |
| `GM重置玩家 @玩家/QQ号` | 重置玩家所有数据 |
| `GM查看玩家 @玩家/QQ号` | 查看玩家详细信息 |
| `GM境界列表` | 查看所有境界及索引 |
| `GM物品列表 [类型]` | 查看物品列表（可选类型筛选） |
| `GM清状态 @玩家/QQ号` | 清除玩家状态（解除闭关/秘境等） |
| `GM添加激活码 <码> [灵石] [修为] [次数] [描述]` | 添加激活码 |
| `GM删除激活码 <激活码>` | 删除激活码 |
| `GM激活码列表` | 查看所有激活码 |
| `GM激活码加物品 <激活码> <物品名> [数量]` | 为激活码添加物品奖励 |

### 激活码系统 (v2.5.0新增)

| 指令 | 说明 |
|:-----|:-----|
| `橘的恩赐 <激活码>` | 使用激活码领取奖励 |

> 激活码可通过GM指令直接添加（推荐），也可在WebUI后台配置

---

## 秘境系统详解 (v2.6.1 全新升级)

秘境系统是一个充满策略性和随机性的探索玩法，玩家需要在不同类型的秘境中做出选择，应对各种事件，最终击败Boss获得丰厚奖励。

### 🎮 基础玩法

#### 如何进入秘境

```
探索秘境 [类型] [难度]
```

**示例：**
- `探索秘境` - 进入试炼之地·普通难度（默认）
- `探索秘境 宝藏` - 进入宝藏密室·普通难度
- `探索秘境 幽冥 地狱` - 进入幽冥鬼域·地狱难度

**进入条件：**
- 消耗灵石：基础 50 + (境界等级 × 25)，根据难度倍率调整
- 不在其他秘境中
- 状态为空闲（未闭关、未探险等）

#### 探索与选择

在秘境中，你会遇到各种事件：

1. **自动事件**（战斗、宝箱、陷阱等）
   - 使用 `前进` 指令继续探索
   - 自动处理事件结果

2. **选择事件**（分岔路口、可疑宝箱、商人）
   - 系统会显示选项列表
   - 使用 `选择 数字` 做出决策
   - 例如：`选择 1` 选择第一个选项

3. **完成或离开**
   - 击败最终Boss完成秘境，获得完成奖励
   - 战斗失败会被传送出秘境
   - 使用 `离开秘境` 主动退出（不获得完成奖励）

### 🏔️ 秘境类型

每种秘境有不同的事件分布和特色：

| 类型 | 名称 | 特点 | 适合玩家 |
|------|------|------|----------|
| **试炼** | 试炼之地 | 平衡型，各类事件分布均匀 | 新手、稳定探索 |
| **宝藏** | 宝藏密室 | 宝箱40%，陷阱20%，高收益高风险 | 追求财富，小心谨慎 |
| **妖兽** | 妖兽巢穴 | 怪物50%，精英15%，战斗密集 | 刷经验、装备 |
| **遗迹** | 古老遗迹 | 神秘事件多，祝福/诅咒/选择 | 追求特殊机遇 |
| **幽冥** | 幽冥鬼域 | 诅咒概率30%，极度危险 | 高手挑战、赌一把 |

### 🔥 难度系统

| 难度 | 灵石消耗倍率 | 奖励倍率 | 说明 |
|------|-------------|----------|------|
| **普通** | 1.0x | 1.0x | 基础难度，适合日常刷取 |
| **困难** | 1.5x | **2.0x** | 收益翻倍，值得挑战 |
| **地狱** | 2.0x | **3.0x** | 收益三倍，高风险高回报 |

> **💡 提示**：困难和地狱难度下，**所有奖励**（灵石、经验、完成奖励）都会乘以倍率！

### 🎲 事件类型详解

秘境中共有 **9种事件类型**：

#### 1. 💀 怪物战斗
- **触发**：自动触发
- **内容**：与境界匹配的怪物战斗
- **奖励**：灵石、经验、材料（凡品/珍品）
- **失败**：生命值归零，被传送出秘境

#### 2. 👹 精英怪物
- **触发**：自动触发（楼层后期概率更高）
- **特点**：生命+30%，攻击+20%，防御+20%
- **奖励**：1.5倍基础奖励
- **说明**：强化版怪物，但奖励丰厚

#### 3. 🐲 Boss战
- **触发**：最终层必定出现
- **特点**：强大的Boss，需要做好准备
- **奖励**：大量灵石、经验、珍品/圣品材料
- **完成**：击败后获得完成奖励并退出秘境

#### 4. 💰 宝箱
- **触发**：自动触发
- **奖励**：灵石（80-200 × 玩家等级 × 难度倍率）
- **特点**：纯收益，无风险

#### 5. ⚡ 陷阱
- **触发**：自动触发（前30%楼层概率降低50%）
- **类型**：
  - 毒雾陷阱：15-25%生命伤害
  - 落石陷阱：20-30%生命伤害
  - 灵力吸收阵：25-35%生命伤害 + 50-150灵石损失
- **说明**：注意生命值，必要时使用回血丹

#### 6. ✨ 祝福/诅咒
- **触发**：自动触发
- **正面效果（祝福）**：
  - 力量祝福：攻击+8，持续5场战斗
  - 守护祝福：防御+5，持续5场战斗
  - 生命祝福：恢复30%生命值
- **负面效果（诅咒）**：
  - 虚弱诅咒：攻击-5，持续3场战斗
  - 破甲诅咒：防御-3，持续3场战斗
- **概率**：幽冥鬼域诅咒概率高达30%

#### 7. 🔀 选择事件
- **触发**：需要玩家做出选择
- **类型一：三岔路口**
  - 选项1：险峻之路（可能战斗/大量灵石/高伤害）
  - 选项2：平坦之路（安全通过/小奖励）
  - 选项3：隐秘之路（小战斗/中等灵石/轻伤）
  
- **类型二：可疑宝箱**
  - 选项1：直接打开（50%大奖/50%陷阱）
  - 选项2：小心检查（安全获得中等灵石）
  - 选项3：不管它（安全通过）

- **操作**：使用 `选择 数字` 做出决定

#### 8. 🛒 商人事件
- **触发**：需要玩家选择是否购买
- **商品**：
  - 疗伤丹药：恢复30%生命，100-150灵石
  - 爆发丹药：攻击+10（3场战斗），150-200灵石
  - 神秘物品：随机凡品/珍品道具，200-300灵石
  - 不购买：直接通过
- **说明**：合理购买可以提升生存率
- **操作**：使用 `选择 数字` 购买或离开

#### 9. 🌟 神秘事件
- **触发**：自动触发（遗迹秘境概率10%）
- **可能结果**：
  - 灵泉：恢复50%生命 + 攻击buff
  - 宝石：大量灵石（300+）
  - 传送阵：受到伤害
  - 蛛网：获得debuff
- **特点**：完全随机，可能好可能坏

### 💎 奖励与掉落

#### 战斗奖励
- **灵石**：50-200 × (1 + 玩家等级 × 0.5) × 难度倍率
- **经验**：根据怪物等级
- **材料**：
  - 普通怪物：凡品材料（灵芝草、火灵草等）
  - 精英怪物：凡品+珍品材料
  - Boss：珍品+圣品材料（龙鳞、凤凰羽等）

#### 完成奖励
击败最终Boss后获得：
- **额外灵石**：200 × (1 + 玩家等级) × 难度倍率

**示例**：
- 玩家等级10，困难难度：200 × 11 × 2 = **4,400灵石**
- 玩家等级10，地狱难度：200 × 11 × 3 = **6,600灵石**

### 📊 楼层机制

秘境楼层数 = 基础楼层（配置） + (玩家等级 ÷ 楼层因子)

**早期楼层**（前30%）：
- 陷阱概率降低50%
- 精英怪物概率降低50%
- 宝箱概率提高20%
- 让玩家平稳过渡

**后期楼层**（后30%）：
- 精英怪物概率提高50%
- 普通怪物概率提高20%
- 难度逐渐提升

**最终层**：
- 必定是Boss战

### 🎯 策略建议

1. **新手推荐**：
   - 类型：试炼之地
   - 难度：普通
   - 携带：一品气血丹 × 3

2. **稳定刷取**：
   - 类型：试炼之地 / 宝藏密室
   - 难度：困难（收益×2）
   - 策略：遇到商人购买回血丹，选择事件选平稳路线

3. **刷经验装备**：
   - 类型：妖兽巢穴
   - 难度：普通/困难
   - 策略：战斗密集，准备充足回血丹

4. **追求特殊机遇**：
   - 类型：古老遗迹
   - 难度：困难/地狱
   - 策略：神秘事件多，可能获得强力buff或珍稀道具

5. **高手挑战**：
   - 类型：幽冥鬼域
   - 难度：地狱
   - 风险：诅咒概率30%，容易受到debuff
   - 收益：3倍奖励，适合实力强大的玩家赌一把

### ⚠️ 注意事项

1. **生命值管理**：
   - 随时关注生命值，低于50%建议使用回血丹
   - 陷阱可能造成高达35%生命伤害
   - Boss战前确保生命值充足

2. **选择事件决策**：
   - 生命值高时可以选择高风险选项
   - 生命值低时选择安全路线
   - 商人处可以购买疗伤丹药应急

3. **待选择事件**：
   - 如果有待选择的事件，必须先做出选择才能前进
   - 使用 `选择 数字` 完成选择后，才能继续 `前进`

4. **离开秘境**：
   - 主动离开不会获得完成奖励
   - 战斗失败会被强制传送出秘境
   - 完成所有楼层击败Boss才能获得最大收益

---

## 游戏机制

### 灵根系统

| 灵根 | 修炼速度 |
|------|---------|
| 五行灵根(金/木/水/火/土) | 1.0x |
| 变异灵根 | 1.2x |
| 天灵根 | 1.5x |
| 融合灵根 | 1.8x |
| 混沌灵根 | 2.0x |

### 境界体系

```
炼气期(5层) → 筑基期(3期) → 搬血境 → 洞天境 → 化灵境 → 铭纹境 →
列阵境 → 尊者境 → 神火境 → 真一境 → 圣祭境 → 天神境 → 虚道境 →
斩我境 → 遁一境 → 至尊境 → 真仙境 → 仙王境 → 准仙帝 → 仙帝境 → 祭道境 → 祭道之上
```

### 丹药中毒系统 (v2.5.3新增)

服用丹药时需注意用量，过量服用会导致丹毒入体！

| 服用数量 | 中毒概率 | 说明 |
|----------|----------|------|
| 1-3 颗 | 0% | 安全范围 |
| 4 颗 | 10% | 开始有风险 |
| 5 颗 | 20% | |
| 6 颗 | 30% | |
| 7 颗 | 40% | |
| 8 颗 | 50% | |
| 9 颗 | 60% | |
| 10+ 颗 | 100% | 必定中毒 |

**中毒效果：**
- 丹药效果完全失效（不获得任何增益）
- 当前血量扣除 50%（至少保留1点血）
- 丹药仍然被消耗

**不受影响的丹药：**
- 逆天改命丹（特殊丹药，只能单颗使用）

### 炼丹/炼器系统

**炼丹师/炼器师等级：**
| 等级 | 称号 | 所需熟练度 |
|------|------|-----------|
| 1级 | 学徒 | 0 |
| 2级 | 初窥门径 | 100 |
| 3级 | 小有所成 | 300 |
| 4级 | 登堂入室 | 600 |
| 5级 | 炉火纯青 | 1000 |
| 6级 | 出神入化 | 1500 |
| 7级 | 宗师 | 2200 |
| 8级 | 大宗师 | 3000 |
| 9级 | 丹圣/器圣 | 4000 |
| 10级 | 丹帝/器帝 | 5500 |

**丹炉/炼器台等级：**
| 等级 | 名称 | 成功率加成 | 品质加成 |
|------|------|-----------|----------|
| 1级 | 凡铁丹炉/简易炼器台 | +0% | +0% |
| 2级 | 青铜丹炉/精工炼器台 | +5% | +5% |
| 3级 | 紫金丹炉/天工炼器台 | +10% | +10% |
| 4级 | 玄天丹炉/神匠炼器台 | +18% | +18% |
| 5级 | 太上丹炉/造化炼器台 | +30% | +30% |

**品质系统：**
| 品质 | 概率 | 效果倍率 |
|------|------|----------|
| 残次 | 15% | 0.7x |
| 普通 | 45% | 1.0x |
| 精良 | 25% | 1.3x |
| 完美 | 12% | 1.6x |
| 传说 | 3% | 2.0x |

### 材料获取途径

| 途径 | 可获得材料品级 |
|------|---------------|
| 商店 | 凡品 |
| 秘境怪物 | 凡品+珍品 |
| Boss掉落 | 珍品+圣品 |
| 奇遇 | 随机 |
| 悬赏任务 | 指定材料 |

---

## 物品图鉴

### 丹药 (17种)

| 名称 | 品级 | 价格 | 效果 |
|------|------|------|------|
| 一品气血丹 | 凡品 | 50 | 恢复100生命 |
| 引气丹 | 凡品 | 200 | 增加100修为 |
| 大力丸 | 凡品 | 80 | 攻击+10 (3场战斗) |
| 解毒丹 | 凡品 | 60 | 恢复50生命 |
| 铁皮丹 | 凡品 | 100 | 防御+10 (3场战斗) |
| 三品回元丹 | 珍品 | 500 | 恢复500生命 |
| 筑基丹 | 珍品 | 1200 | 攻击+30 (5场战斗) |
| 聚灵丹 | 珍品 | 1000 | 增加500修为 |
| 虎力丹 | 珍品 | 800 | 攻击+25 (5场战斗) |
| 金钟丹 | 珍品 | 1800 | 防御+30 (5场战斗) |
| 续命丹 | 珍品 | 800 | 恢复800生命 |
| 五品凝神丹 | 圣品 | 8000 | 增加5000修为 |
| 九转还魂丹 | 圣品 | 15000 | 恢复8000生命 |
| 破障丹 | 圣品 | 25000 | 防御+100 (8场战斗) |
| 洗髓丹 | 圣品 | 12000 | 增加8000修为 |
| 金髓丹 | 帝品 | 100000 | 增加50000修为 |
| 逆天改命丹 | 帝品 | 50000 | 重置灵根 |

### 材料 (24种)

| 名称 | 品级 | 价格 | 用途 |
|------|------|------|------|
| 灵芝草 | 凡品 | 50 | 炼制回血丹药 |
| 火灵草 | 凡品 | 60 | 炼制攻击类丹药 |
| 寒玉花 | 凡品 | 60 | 炼制防御类丹药 |
| 聚气草 | 凡品 | 70 | 炼制修为丹药 |
| 百年灵草 | 凡品 | 80 | 基础炼丹材料 |
| 妖兽皮 | 凡品 | 80 | 制作防具 |
| 妖兽骨 | 凡品 | 90 | 锻造武器 |
| 精铁 | 凡品 | 100 | 锻造凡品武器 |
| 玄铁 | 凡品 | 120 | 锻造防具 |
| 丹火石 | 凡品 | 30 | 炼丹燃料 |
| 血灵芝 | 珍品 | 600 | 炼制高级回血丹 |
| 虎魄 | 珍品 | 800 | 炼制攻击类丹药 |
| 玄龟壳 | 珍品 | 900 | 炼制防御类丹药/防具 |
| 九叶莲 | 珍品 | 1000 | 炼制突破类丹药 |
| 灵蚕丝 | 珍品 | 1000 | 制作法袍 |
| 寒铁 | 珍品 | 1400 | 锻造冰属性法器 |
| 妖丹碎片 | 珍品 | 1500 | 炼丹催化剂 |
| 紫金沙 | 珍品 | 1500 | 炼制高级法器 |
| 聚灵阵盘 | 珍品 | 500 | 提升炼制成功率 |
| 玄冰之核 | 圣品 | 8000 | 极致寒冰之力 |
| 天火精华 | 圣品 | 10000 | 炼制极品攻击丹药 |
| 万年玄冰 | 圣品 | 10000 | 炼制极品防御丹药 |
| 天外陨铁 | 圣品 | 12000 | 锻造圣品武器 |
| 龙鳞 | 圣品 | 15000 | 锻造圣品防具 |
| 凤凰羽 | 圣品 | 18000 | 锻造圣品饰品 |
| 龙晶石 | 圣品 | 20000 | 锻造龙系法器 |
| 仙灵之露 | 帝品 | 50000 | 炼制帝品丹药 |
| 混沌精金 | 帝品 | 80000 | 锻造帝品法器 |

### 法器-武器 (8种)

| 名称 | 品级 | 价格 | 攻击加成 |
|------|------|------|----------|
| 青锋剑 | 凡品 | 800 | +8 |
| 玄铁刀 | 凡品 | 1000 | +12 |
| 烈阳刀 | 珍品 | 4000 | +25 |
| 妖精之弓 | 圣品 | 550000 | +220 |
| 龙渊剑 | 圣品 | 580000 | +230 |
| 寒霜剑 | 圣品 | 600000 | +250 |
| 弑神枪 | 帝品 | 9999999 | +500 |

### 法器-防具 (5种)

| 名称 | 品级 | 价格 | 防御/生命加成 |
|------|------|------|---------------|
| 玄铁甲 | 凡品 | 800 | 防御+10 |
| 灵蚕袍 | 珍品 | 3500 | 防御+18, 生命+30 |
| 月华袍 | 珍品 | 4000 | 防御+22, 生命+50 |
| 龙鳞铠 | 圣品 | 650000 | 防御+280, 生命+150 |
| 泰坦之铠 | 圣品 | 750000 | 防御+300, 生命+200 |

### 法器-饰品 (5种)

| 名称 | 品级 | 价格 | 属性加成 |
|------|------|------|----------|
| 聚灵珠 | 凡品 | 600 | 攻击+3, 防御+3 |
| 护心镜 | 珍品 | 3000 | 防御+15, 生命+50 |
| 凤凰玉佩 | 圣品 | 480000 | 生命+300, 防御+80 |
| 镇魂幡 | 圣品 | 500000 | 攻击+100, 防御+50 |
| 戮仙剑阵 | 帝品 | 8888888 | 攻击+800, 防御+200 |

### 功法 (10种)

| 名称 | 品级 | 价格 | 永久属性加成 |
|------|------|------|--------------|
| 龟息功 | 凡品 | 900 | 防御+8 |
| 长春功残篇 | 凡品 | 1000 | 生命+50 |
| 御风诀 | 凡品 | 1200 | 攻击+8 |
| 不动明王经 | 珍品 | 10000 | 防御+30, 生命+100 |
| 九阳神功 | 珍品 | 22000 | 生命+150, 防御+25 |
| 北冥神功 | 珍品 | 25000 | 攻击+50 |
| 焚天诀上卷 | 圣品 | 888888 | 攻击+150 |
| 吞天魔功 | 帝品 | 6666666 | 攻击+400 |
| 道经 | 帝品 | 7777777 | 攻击+250, 防御+150, 生命+400 |
| 他化自在大法 | 帝品 | 9999999 | 攻击+300, 防御+200, 生命+500 |

---

## 配置说明

### WebUI 配置

通过 AstrBot WebUI 的插件配置页面可调整：

| 配置项 | 描述 |
|--------|------|
| `ACCESS_CONTROL.WHITELIST_GROUPS` | 白名单群号列表，留空表示所有群可用 |
| `VALUES.INITIAL_GOLD` | 新玩家初始灵石数量 |
| `VALUES.CHECK_IN_REWARD_MIN/MAX` | 签到奖励范围 |
| `VALUES.BASE_EXP_PER_MINUTE` | 闭关每分钟获得的基础修为 |
| `VALUES.CREATE_SECT_COST` | 创建宗门消耗灵石 |
| `VALUES.SHOP_DAILY_ITEM_COUNT` | 每日商店刷新商品数量 |
| `SPIRIT_ROOT_SPEEDS.*` | 各灵根修炼速度倍率 |

### 配置文件

| 文件 | 用途 |
|------|------|
| `config/level_config.json` | 境界配置（名称、升级修为、成功率） |
| `config/items.json` | 物品配置（名称、价格、效果） |
| `config/monsters.json` | 怪物模板配置 |
| `config/bosses.json` | Boss模板配置 |
| `config/tags.json` | 怪物标签系统（属性修饰、掉落） |
| `config/recipes.json` | 炼丹/炼器配方配置 |

---

## 项目结构

```
astrbot_plugin_xiuxian/
├── main.py                 # 主入口，注册所有指令
├── models.py               # 数据模型（Player, Item, Monster等）
├── config_manager.py       # 配置加载管理器
├── handlers/               # 指令处理器
│   ├── player_handler.py   # 玩家相关
│   ├── shop_handler.py     # 商店/背包/出售
│   ├── combat_handler.py   # 战斗
│   ├── sect_handler.py     # 宗门
│   ├── realm_handler.py    # 秘境
│   ├── crafting_handler.py # 炼丹/炼器
│   └── ...
├── core/                   # 核心业务逻辑
│   ├── cultivation_manager.py  # 修炼管理
│   ├── combat_manager.py       # 战斗计算
│   ├── realm_manager.py        # 秘境生成
│   ├── sect_manager.py         # 宗门管理
│   └── crafting_manager.py     # 炼丹/炼器管理
├── data/                   # 数据层
│   ├── data_manager.py     # 数据库操作
│   └── migration.py        # 数据库迁移（v15）
└── config/                 # 游戏配置JSON
    ├── items.json          # 物品配置
    ├── recipes.json        # 配方配置
    └── ...
```

---

## 数据库

- **存储引擎**：SQLite (aiosqlite异步)
- **数据库版本**：v23
- **存储位置**：`AstrBot/data/xiuxian/xiuxian_data.db`
- **自动迁移**：插件升级时自动执行数据库迁移

---

## 更新日志

### v2.8.0 (最新) - 世界Boss系统增强

**⚔️ Boss系统全面升级**：
- **Boss冷却生效**：Boss被击杀后根据配置冷却时间重新生成，不再立即刷新
- **玩家攻击冷却**：每位玩家对同一Boss有攻击冷却（默认2小时），防止独吞
- **难度大幅提升**：Boss基础属性翻倍，新增难度系数配置（默认3倍）

**🏆 奖励系统改进**：
- **物品奖励发放**：首功玩家获得Boss掉落的物品奖励
- **伤害排名额外奖励**：前三名获得额外灵石和经验（🥇🥈🥉标识）
- **奖励更丰厚**：灵石和经验基础值提升60%

**📜 Boss战报系统**：
- 新增 `boss战报` 指令查看近期10条击杀记录
- 记录击杀时间和功勋榜前三
- Boss击杀后自动推送战报到指定群

**🎮 新增指令**：
- `boss战报` - 查看近期Boss击杀战报

**⚙️ 新增配置项**：
- `WORLD_BOSS_PLAYER_COOLDOWN_MINUTES`：玩家攻击冷却（默认120分钟）
- `WORLD_BOSS_DIFFICULTY_MULTIPLIER`：Boss难度系数（默认3.0）
- `WORLD_BOSS_RANK_BONUS_GOLD`：排名额外灵石奖励
- `WORLD_BOSS_RANK_BONUS_EXP`：排名额外经验奖励
- `WORLD_BOSS_BROADCAST_GROUP`：Boss击杀战报推送群号

**📦 数据库更新**：
- 数据库版本：v22 → v23
- 新增 `world_boss_kill_logs` 表（击杀记录）
- `active_world_bosses` 新增 `defeated_at` 字段
- `world_boss_participants` 新增 `last_attack_at` 字段

### v2.7.1 - 宗门建筑系统

**🏛️ 宗门建筑上线**：
- 藏经阁：全员修炼速度加成 (+15%~35%)
- 炼丹房：全员炼丹成功率加成 (+8%~18%)
- 灵田：每日产出灵石归宗门

**⚙️ 建筑功能**：
- 建筑分3级，消耗宗门资金+灵晶升级
- 激活建筑可为全体成员添加Buff
- 每日可激活次数随等级提升

**🛠️ 宗门商店修复**：
- 修复物品显示乱码问题 (� → ✦)
- 补全所有效果类型处理：
  - 随机材料礼包、修炼加速Buff
  - 突破成功率加成、永久属性加成
  - 宗门建材、随机物品福袋

**🎮 命令新增**：
- `宗门建筑` - 查看宗门所有建筑
- `建造建筑 <名>` - 建造新建筑(宗主)
- `升级建筑 <名>` - 升级建筑(宗主)
- `激活建筑 <名>` - 激活建筑Buff(全员生效)

### v2.7.0 - 宗门商店系统

**🏪 宗门商店上线**：
- 宗门成员可用贡献度兑换独家物品
- 10种商品：培元丹、破境丹、战神丸、聚灵散、材料包、功法等
- 部分物品每日限购，防止囤积
- 需要宗门等级解锁高级物品

**📊 设计亮点**：
- 贡献度终于有用了！
- 宗门专属物品，性价比合理
- 聚灵散：修炼速度+30% (60分钟)
- 破境丹：突破成功率+20%

**📦 数据库更新**：
- 数据库版本：v21 → v22
- 新增 `sect_shop_daily_limit` 表（限购记录）
- 预留 4 张表为未来宗门功能扩展

**🎮 命令新增**：
- `宗门商店` - 查看可兑换的物品
- `兑换 <物品名> [数量]` - 消耗贡献度兑换

**🛣️ 后续计划** (v2.7.1-v2.7.3)：
- 宗门建筑系统
- 宗门每日任务
- 宗门排行榜

### v2.6.6 - GM指令修复

**🔧 Bug修复**:
- **GM指令参数解析错误**：修复GM加灵石、GM加修为、GM设境界、GM加物品、GM设生命等指令无法正确接收参数的问题
  - 修复前：`/GM设生命 1733533842 99999` 只能读取到QQ号，数值被丢弃
  - 修复后：正确解析QQ号和数值两个参数
  - 影响命令：GM加灵石、GM加修为、GM设境界、GM加物品、GM设生命

### v2.6.5 - 道具平衡与限购系统

**⚠️ 道具价格平衡调整：**
- **虎力丹**：1,500 → 800 灵石
  - 性价比过高问题修复
  - 效果：攻击+25（5场战斗）
  - 新性价比：32灵石/场
  
- **筑基丹**：2,000 → 1,200 灵石
  - 性价比过高问题修复
  - 效果：攻击+30（5场战斗）
  - 新性价比：240灵石/场

**🛒 道具限购系统：**
- **九转还魂丹**：每日限购 **1个**
- **金髓丹**：每日限购 **2个**
- **洗髓丹**：每日限购 **3个**
- 0点刷新限购次数
- 购买时显示剩余额度
- 防止玩家囤积过多高级丹药

**📦 数据库更新：**
- 数据库版本：v20 → v21
- 新增 `daily_item_purchase` 表（道具购买记录）
- 自动迁移，玩家无需手动操作

**👁️ 体验改进：**
- 购买限购道具时显示剩余额度
- 超限时友好提示
- Buff丹药价格更合理

### v2.6.4

**🔴 关键问题修复：**
- **天劫每日3次限制**：修复天劫系统无限刷漏洞
  - 每位玩家每日只能渡劫3次
  - 0点刷新次数
  - 防止玩家反复刷修为和灵石

- **秘境每日10次限制**：修复秘境系统无限刷漏洞
  - 每位玩家每日最多探索10次秘境
  - 0点刷新次数
  - 防止灵石和材料过度产出，保护游戏经济平衡

**⚠️ 道具平衡性调整：**
- **九转还魂丹**：99999 HP → 8000 HP
  - 修复性价比过高问题
  - 由“瞬间恢复所有生命”改为“恢复大量生命”
  - 仍然是最强的回血丹药，但不再不平衡

**📦 数据库更新：**
- 数据库版本：v19 → v20
- 新增 `daily_tribulation_count` 表（天劫次数记录）
- 新增 `daily_realm_count` 表（秘境次数记录）
- 自动迁移，玩家无需手动操作

**👁️ 体验改进：**
- 渡劫/秘境失败时显示剩余次数
- 人性化提示，帮助玩家规划每日任务

> 🚨 **重要提示**：此版本修复了严重影响游戏平衡的漏洞，建议立即更新！

### v2.6.3

**Bug修复：**
- **数据库迁移修复 v2**：修复 v2.6.2 中数据库版本号与实际schema不同步导致迁移被跳过
- **数据库版本**：v18 → v19，增加防御性检查，确保 `realm_pending_choice` 字段存在
- 现在即使版本号已是v18，也会检查并添加缺失的字段

### v2.6.2

**Bug修复：**
- **数据库迁移修复**：修复 v2.6.1 中缺少数据库迁移导致玩家无法进入秘境（`realm_pending_choice` 字段缺失）
- **数据库版本**：v17 → v18，自动为现有玩家添加秘境选择字段

### v2.6.1

**秘境系统重新设计 - 重大更新**

**新增功能：**
- **8种事件类型**：不再只是战斗和宝箱
  - 普通怪物：基础战斗
  - 精英怪物：强化版怪物(属性+20-30%)，奖励1.5倍
  - Boss：最终层强大Boss
  - 宝箱：获得灵石奖励
  - 陷阱：可能造成伤害和灵石损失
  - 祝福/诅咒：随机获得正面或负面buff效果
  - 选择事件：玩家需做出策略选择（分岔路口、可疑宝箱等）
  - 商人事件：购买道具或服务
  - 神秘事件：随机好坏事件

- **5种主题秘境**：每种主题有不同的事件分布
  - 试炼之地：平衡型，适合稳定探索
  - 宝藏密室：宝箱丰富(40%)，但陷阱众多(20%)
  - 妖兽巢穴：战斗密集(50%)，精英怪多(15%)
  - 古老遗迹：神秘事件多(10%)，可能获得珍稀奖励
  - 幽冥鬼域：极度危险，奖励丰厚，诅咒概率高(30%)

- **3个难度等级**：风险与收益的平衡
  - 普通：消耗1.0x，奖励1.0x
  - 困难：消耗1.5x，奖励2.0x
  - 地狱：消耗2.0x，奖励3.0x

- **策略选择系统**：
  - 分岔路口：选择高风险高回报、平衡或安全路线
  - 可疑宝箱：直接开(可能陷阱或大奖)、小心检查或放弃
  - 商人交易：购买疗伤丹药、buff丹药或随机物品

**玩法改进：**
- 秘境不再是无脑刷怪，需要策略决策和资源管理
- 楼层难度动态调整：前30%楼层降低陷阱和精英概率
- 完成奖励：根据难度给予额外灵石奖励
- 事件描述更生动，增强沉浸感

**新增指令：**
- `探索秘境 [类型] [难度]` - 指定秘境类型和难度
  - 示例：`探索秘境` (默认试炼普通)  
  - 示例：`探索秘境 宝藏 困难` (宝藏密室困难难度)
- `选择 <数字>` - 在选择事件或商人事件中做出选择

**技术改进：**
- 新增 `realm_events.py` 模块：事件生成和处理逻辑
- 重构 `RealmGenerator`：支持主题和难度
- 扩展数据模型：支持事件选择和待选择状态
- 优化奖励计算：应用难度倍率和主题修正

### v2.5.3

**新增机制：**
- **丹药中毒系统**：防止玩家一次性服用大量丹药
  - 单次服用 1-3 颗丹药：安全，无中毒风险
  - 单次服用 4-9 颗丹药：中毒概率递增（4颗10%，5颗20%...9颗60%）
  - 单次服用 10 颗及以上：必定中毒（100%）
  - 中毒效果：丹药效果完全失效 + 当前血量扣除50%
  - 适用于所有丹药类型（buff丹药、回血丹、修为丹等）
  - 特殊丹药（如逆天改命丹）不受影响

### v2.5.2

**Bug修复：**
- **GM命令支持QQ号**：所有GM指令现在支持@玩家和直接输入QQ号两种方式
  - 示例：`GM加灵石 @玩家 1000` 或 `GM加灵石 123456789 1000`
- **血量恢复上限修复**：修复使用增加血量上限的道具（装备/功法）后，回血无法恢复到实际最大血量的问题
  - 现在回血道具会正确计算装备和功法带来的血量上限加成
- **GM命令参数解析修复**：修复GM命令中@用户和数值参数混淆导致的解析错误

### v2.5.0

**新增系统：**
- **GM管理员工具**：完整的游戏数据管理指令集
  - 灵石/修为增减
  - 境界设置
  - 物品添加
  - 生命值设置
  - 玩家数据重置
  - 玩家详情查看
  - 状态清除（解除闭关/秘境卡住）
- **激活码系统（橘的恩赐）**：玩家补偿/福利发放系统
  - 支持中文激活码
  - 可配置最大使用次数
  - 可赠送灵石、修为、物品
  - WebUI后台可视化配置

**新增指令：**
- `GM加灵石 @玩家 <数额>` - 增减灵石
- `GM加修为 @玩家 <数额>` - 增减修为
- `GM设境界 @玩家 <索引>` - 设置境界
- `GM加物品 @玩家 <物品名> [数量]` - 添加物品
- `GM设生命 @玩家 <数值>` - 设置生命值
- `GM重置玩家 @玩家` - 重置玩家数据
- `GM查看玩家 @玩家` - 查看详细信息
- `GM境界列表` - 查看境界索引
- `GM物品列表 [类型]` - 查看物品列表
- `GM清状态 @玩家` - 清除玩家状态
- `橘的恩赐 <激活码>` - 使用激活码领取奖励

### v2.4.0

**新增系统：**
- **炼丹系统**：收集材料炼制丹药，16种丹药配方
- **炼器系统**：收集材料锻造法器，18种炼器配方
- **炼丹师/炼器师等级**：10级等级体系，提升成功率
- **丹炉/炼器台升级**：5级升级体系，提升成功率和品质
- **品质系统**：残次/普通/精良/完美/传说五种品质
- **物品出售**：每日5次回购机会，40%价格出售物品

**新增物品：**
- 24种新材料（灵芝草、火灵草、寒玉花、聚气草等）
- 6种新丹药（虎力丹、铁皮丹、金钟丹等）
- 7种新法器（玄铁刀、灵蚕袍、聚灵珠等）

**新增指令：**
- `炼丹 [配方名]` - 炼丹界面/炼制丹药
- `炼器 [配方名]` - 炼器界面/炼制法器
- `升级丹炉` - 升级丹炉
- `升级炼器台` - 升级炼器台
- `配方 <配方名>` - 查看配方详情
- `配方图鉴` - 查看所有配方
- `材料图鉴` - 查看所有材料
- `出售 <物品名> [数量]` - 出售物品

**优化改进：**
- 怪物掉落增加装备和功法
- 材料现在有实际用途（炼丹/炼器）
- 物品获取途径更加多样化

### v2.3.0

**新增系统：**
- 功法系统：学习功法永久提升攻击/防御/生命
- Buff系统：丹药提供临时战斗加成
- 奇斗系统：带灵石赌注的PVP
- 玩家交易：支持转账灵石和赠送物品
- 宗门捐献：贡献灵石获得宗门贡献度
- PVP排行：新增胜场/胜率排行榜

### v2.2.0

**新增系统：**
- 排行榜系统（境界/财富/战力三榜）
- 每日任务系统（5任务+全勤奖励）
- 奇遇系统（每日3次，10种奇遇事件）
- 天劫系统（4重天劫挑战）
- 悬赏任务系统（7种难度）

---

## 后续规划

- [ ] **宗门扩展**：宗门任务、宗门仓库、宗门等级
- [ ] **灵宠系统**：捕捉、培养、战斗辅助
- [ ] **洞府系统**：个人洞府、灵田种植
- [x] **管理员工具**：GM指令修改游戏数据 ✅ v2.5.0已实现
- [ ] **钱庄系统**：灵石存储利息

---

## 贡献

欢迎通过以下方式贡献：

- 提交 Issue 报告 Bug
- 提出功能建议
- 提交 Pull Request

---

## 许可证

本项目采用 [AGPL-3.0](LICENSE) 许可证开源。
